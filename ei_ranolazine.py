...
from utils.utils_final import evaluate_ranol_MPO
...
def bayesian_optimization_loop(
    ...
    for iteration in range(n_iterations):
        ...
        if best_smiles:
            ...
            new_Y = evaluate_ranol_MPO([best_smiles])
            ...
            # Store the chosen SMILES value (predicted mean)
            results.append([iteration + 1, best_mean, best_value])
        else:
...
def write_to_csv(results, filename):
    with open(filename, mode="w", newline="") as file:
        ...
        writer.writerow(["Experiment", "Iteration", "Predicted Mean Value", "Best Value"])
        for row in results:
            writer.writerow(row)
...
def run_experiment(repeats, n_iterations):
    ...
    for experiment_num in range(1, repeats + 1):
        ...
        hyperparam_Y = evaluate_ranol_MPO(hyperparam_smiles)
        training_Y = evaluate_ranol_MPO(training_smiles)
...
if __name__ == "__main__":
    ...
    write_to_csv(results, "ei_bo_experiments_ranolazine_results.csv")
