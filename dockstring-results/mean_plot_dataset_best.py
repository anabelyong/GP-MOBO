...
# Load the dataset_best.csv to get the 20 SMILES values
dataset_best_df = pd.read_csv('dataset_best.csv')
dataset_best_values = dataset_best_df['Value of Best SMILES'].values
...
# Plot the "Dataset BEST mean" line
plt.plot(iterations, dataset_best_values, label='20 Best SMILES Values', linestyle=':', color='black', marker='o', linewidth=2)

plt.xlabel('BO Iteration')
plt.ylabel('Geometric mean of Chosen SMILEs')
plt.xticks(range(0, 20))  
plt.yticks(np.arange(0.0, 2.4, 0.10)) 
plt.title('Average Value of Chosen SMILEs for 20 BO Iterations in 3 experiments')
plt.legend()
plt.grid(True)

# Save the plot as a PDF file
plt.savefig('chosen_smiles_values_with_dataset_best.pdf', format='pdf')
