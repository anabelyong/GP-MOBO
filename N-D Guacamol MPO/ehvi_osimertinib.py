...
from utils.utils_final import evaluate_osim_objectives
...
def bayesian_optimization_loop(
    ...
    for iteration in range(n_iterations):
        ...
        if best_smiles:
            ...
            new_Y = evaluate_osim_objectives([best_smiles])
            ...
            # Store the value of the chosen SMILES (f1, f2, f3, f4 from evaluation)
...
def run_experiment(repeats, n_iterations):
    ...
    for experiment_num in range(1, repeats + 1):
        ...
        hyperparam_Y = evaluate_osim_objectives(hyperparam_smiles)
        training_Y = evaluate_osim_objectives(training_smiles)

        # Calculate GP hyperparameters from the training set
        gp_means = np.mean(hyperparam_Y, axis=0)
        gp_amplitudes = np.maximum(np.var(hyperparam_Y, axis=0))
        gp_noises = np.maximum(np.var(hyperparam_Y, axis=0) * 0.1)  
...
def write_results_to_csv(results, filename):
    with open(filename, mode='w', newline='') as file:
        ...
        writer.writerow(["Experiment", "BO Iteration", "EHVI-OSIM-MPO-F1", "EHVI-OSIM-MPO-F2", "EHVI-OSIM_MPO-F3", "EHVI-OSIM-MPO-F4"])
        for row in results:
            writer.writerow(row)
...
if __name__ == "__main__":
    ...
    write_results_to_csv(results, 'ehvi_bo_experiments_osimertinib_results.csv')
