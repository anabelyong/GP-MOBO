...
def infer_reference_point(
    ...
    if max_ref_point is not None:
        ...
        better_than_ref = (pareto_Y[:, non_nan_idx] > max_ref_point[non_nan_idx]).all(dim=-1)
    else:
        non_nan_idx = torch.ones(pareto_Y.shape[-1], dtype=torch.bool, device=pareto_Y.device)
        better_than_ref = torch.ones(pareto_Y.shape[:1], dtype=torch.bool, device=pareto_Y.device)
    if max_ref_point is not None and better_than_ref.any() and non_nan_idx.all():
        ...
        if max_ref_point is not None:
            nadir[non_nan_idx] = torch.min(nadir[non_nan_idx], max_ref_point[non_nan_idx])
...
class Hypervolume:
    ...
    def compute(self, pareto_Y: Tensor) -> float:
        ...
        if pareto_Y.shape[-1] != self._ref_point.shape[0]:
            ...
            raise BotorchTensorDimensionError(f"pareto_Y must have exactly two dimensions, got {pareto_Y.ndim}.")
        ...
        return self._hv_recursive(i=self._ref_point.shape[0] - 1, n_pareto=pareto_Y.shape[0], bounds=bounds)
    ...
    def _initialize_multilist(self, pareto_Y: Tensor) -> None:
        ...
        """
        m = pareto_Y.shape[-1]
        nodes = [Node(m=m, dtype=pareto_Y.dtype, device=pareto_Y.device, data=point) for point in pareto_Y]
...
class SubsetIndexCachingMixin:
    ...
    def compute_q_subset_indices(self, q_out: int, device: torch.device) -> BufferDict[str, Tensor]:
...
def compute_subset_indices(q: int, device: Optional[torch.device] = None) -> BufferDict[str, Tensor]:
    ...
    return BufferDict(
        {
            f"q_choose_{i}": torch.tensor(list(combinations(indices, i)), dtype=torch.long, device=device)
...
class NoisyExpectedHypervolumeMixin(CachedCholeskyMCSamplerMixin):
    ...
    def _set_cell_bounds(self, num_new_points: int) -> None:
        ...
        if self.X_baseline.shape[0] > 0:
            ...
            sample_shape = self.sampler.sample_shape if self.sampler is not None else self._default_sample_shape
            obj = torch.empty(
                *sample_shape,
                0,
        ...
        if self.constraints is None or self.X_baseline.shape[0] == 0:
            ...
            feas = compute_feasibility_indicator(constraints=self.constraints, samples=samples)
        ...
        if self.ref_point.shape[-1] > 2:
            ...
            for sample in obj_cpu:
                partitioning = self.p_class(ref_point=ref_point_cpu, Y=sample, **self.p_kwargs)
            ...
            obj = _pad_batch_pareto_frontier(
                Y=obj,
                ref_point=self.ref_point.unsqueeze(0).expand(obj.shape[0], self.ref_point.shape[-1]),
                feasibility_mask=feas,
            )
            self.partitioning = self.p_class(ref_point=self.ref_point, Y=obj, **self.p_kwargs)
                ...
                if num_new_points > 0:
                    if num_new_points > self._max_iep:
                        ...
                        if not self.incremental_nehvi:
                            self._prev_nehvi = (self._hypervolumes - self._initial_hvs).clamp_min(0.0).mean()
