...
if __name__ == "__main__":
    ...
    def run_test_case(reference, points, expected):
        output = estimate_hypervolume_via_sampling(reference=reference, points=points, rng=rng)
        print(f"Reference: {reference}, Points: {points}")
        print(f"Expected: {expected}, Output: {output}")
        assert math.isclose(output, expected, rel_tol=1e-2), f"{output} vs {expected}"
